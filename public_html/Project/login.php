     ...
     if(count($errors) > 0){
        ...
        try{
            ...
            if($r){
                ...
                //check if we got the user, this return false if no records matched
                if($user) {
                    $hash = $user["password"];
                    //remove password from the user object so it doesn't leave scope(avoids passwords leakage in code)
                    unset($user["password"]);
